    ...
    <section className="py-12 md:py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-8">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-navy-1 mb-3">
            Why Our Assignment Help Online Is Every Student's First Choice
          </h2>
          <p className="text-sm md:text-base text-navy-3 max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          {/* First 6 Items - Normal Grid */}
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {features.slice(0, 6).map((feature, index) => (
              <div 
                key={index}
                className="bg-white-6 rounded-xl p-4 border-2 border-transparent hover:border-blue-4 hover:shadow-xl transition-all duration-300 group"
              >
                {/* Icon (Smaller) */}
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-navy-2 to-navy-3 flex items-center justify-center text-white mb-3 group-hover:scale-110 transition-transform duration-300">
                  {feature.icon}
                </div>

                {/* Title (Smaller) */}
                <h3 className="text-base font-bold text-navy-1 mb-2">
                  {feature.title}
                </h3>

                {/* Description (Smaller) */}
                <p className="text-navy-3 text-xs leading-relaxed">
                  {feature.description}
                </p>
              </div>
            ))}
          ...
          <div className="flex justify-center mt-4">
            <div className="w-full md:w-1/2 lg:w-1/3">
              <div className="bg-white-6 rounded-xl p-4 border-2 border-transparent hover:border-blue-4 hover:shadow-xl transition-all duration-300 group">
                {/* Icon (Smaller) */}
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-navy-2 to-navy-3 flex items-center justify-center text-white mb-3 group-hover:scale-110 transition-transform duration-300">
                  {features[6].icon}
                </div>
        ...
        {/* CTA Button (Compact) - Now Links to /order */}
        <div className="text-center">
          <Link 
            to="/order"
            className="bg-white text-navy-1 px-8 py-3 rounded-full font-bold text-base shadow-xl hover:shadow-2xl hover:scale-105 transition-all duration-300 border-2 border-navy-1 inline-flex items-center gap-2 group"
          >
